...
#    test_order_many_occuer = test_order.groupby(by=['id']).apply(lambda x:x['sts_order'].count()).reset_index(name = '_order_many_occuer')
    ...
    #%%usesr_birthday
    tmp_tmp = train_user[['id','birthday']];tmp_tmp = tmp_tmp.set_index(['id'])
    #转变tmp_tmp中列索引“id”由object转为float类型
    is_double_ = tmp_tmp['birthday'].map(lambda x:(str(x) == '--')*1).reset_index(name='is_double_')
    is_0_0_0 = tmp_tmp['birthday'].map(lambda x:(str(x) == '0-0-0')*1).reset_index(name='is_0_0_0')
    is_1_1_1 = tmp_tmp['birthday'].map(lambda x:(str(x) == '1-1-1')*1).reset_index(name='is_1_1_1')
    is_0000_00_00 = tmp_tmp['birthday'].map(lambda x:(str(x) == '0000-00-00')*1).reset_index(name='is_0000_00_00')
...
#%%
    ...
    #test_data.to_csv("E:\\git_project\\Xgboost-study_User-loan-risk-prediction\\model1\\test_data.csv")
    #is_double_.to_csv("E:\\git_project\\Xgboost-study_User-loan-risk-prediction\\model1\\is_double_.csv")
    #tmp_tmp.to_csv("E:\\git_project\\Xgboost-study_User-loan-risk-prediction\\model1\\tmp_tmp.csv")
    test_data['id'] = test_data['id'].astype("float64")
    is_double_['id'] = is_double_['id'].astype("float64")
    is_0_0_0['id'] = is_0_0_0['id'].astype("float64")
    is_1_1_1['id'] = is_1_1_1['id'].astype("float64")
    is_0000_00_00['id'] =  is_0000_00_00['id'].astype("float64")
    is_0001_1_1['id'] = is_0001_1_1['id'].astype("float64")
    is_hou_in['id'] = is_hou_in['id'].astype("float64")
